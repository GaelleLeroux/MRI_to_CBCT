    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, unused=0):
        if batch_idx % self.log_steps == 0:
            with torch.no_grad():
                ...
                mri, cbct = batch  # Supposons que le batch contient des images CBCT et MRI directement
                ...
                if cbct.ndim == 4:  # (Batch, Channels, Height, Width)
                    cbct_image = cbct[0].cpu().numpy().squeeze()
                else:  # (Batch, Height, Width)
                    cbct_image = cbct[0].cpu().numpy()

                if mri.ndim == 4:  # (Batch, Channels, Height, Width)
                    mri_image = mri[0].cpu().numpy().squeeze()
                else:  # (Batch, Height, Width)
                    mri_image = mri[0].cpu().numpy()

                ax1.imshow(cbct_image, cmap='gray')
                ax1.set_title('CBCT Slice')
                ax2.imshow(mri_image, cmap='gray')
                ax2.set_title('MRI Slice')
                
                # Vérifiez que le logger supporte l'upload des figures
                try:
                    run = trainer.logger.experiment
                    run.wait()  # Forcer la synchronisation des données
                    run["images/cbct_mri_comp"].upload(fig)
